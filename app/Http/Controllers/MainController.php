...
class MainController extends Controller
{
    ...
    public function wishlist_show()
    {
        ...
        if (auth()->check()) {
            ...
            $data = Wishlist::with(['variant','simple_product'])->with('variant.variantimages')->whereHas('variant')->with('variant.products')
            ->whereHas('variant.products', function($query){

                return $query->where('status','=','1');

            })->orWhereHas('simple_product')->where('user_id', auth()->id())->get();
