...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'simple_featuredproducts' => $this->simple_products($type = 1),
            'date' => now(),
    ...
    public function simple_products(){
        ...
        })->where('status','=','1')->where('featured', '=', '1')->orderBy('id', 'DESC')->get();
        ...
        return $simple_products->map(function($content) use($conversion_rate) {
            ...
            $content['mainprice'] = (float) sprintf("%.2f", $content->price * $conversion_rate);

            $content['offerprice'] = (float) sprintf("%.2f", $content->offer_price * $conversion_rate);
            ...
            foreach ($top_categories->category_ids as $category) {
                ...
                if (isset($category) && $category->status == '1') {
                    ...
                    $result[] = array(
                        ...
                        'simple_products' => $this->tabbed_simple_products($category->id),
                    );
    ...
    public function getTabbedProducts()
    {
        ...
        $all = array(
            ...
            'simple_products' => $this->tabbed_simple_products('all'),
        );
    ...
    public function totalCart()
    {
        ...
        try {
            ...
            return response()->json([
                ...
                'count' => $count, 
                'shipping' => (float) sprintf("%.2f",$shipping * $conversion_rate), 
    ...
    public function digitalproducts(){
        ...
        $digitalproducts = SimpleProduct::where('status','1')->get();

        $digitalproducts->map(function($item){

            if($item->offer_price != ''){

                $offer_price_tax = $item->offer_price * $item->tax / 100;

                $item['offer_price'] = (float) sprintf("%2.f",$item->offer_price + $offer_price_tax);

            }

            $pricetax = $item->price * $item->tax / 100;

            $item['price'] = (float) sprintf("%2.f",$item->price + $pricetax);

            $item['position'] = session()->get('currency')['position'];

            $item['thumbnail'] = url('images/digital_products/'.$item->thumbnail);
            $item['hover_thumbnail'] = url('images/digital_products/'.$item->hover_thumbnail);
            
            $item['pricein'] = session()->get('currency')['id'];
            $item['symbol'] = session()->get('currency')['value'];

            $item['url'] = route('show.digitalproduct',['id' => $item->id,'slug' => $item->slug]);
