                       ...
                       <div class="row">
                            ...
                            <div class="col-11 col-md-11 col-xl-11">
                              <div class="checkout-page-dtl">
                                      ...
                                      @if($row->ori_offer_price != '')
                                        ...
                                        <i class="{{session()->get('currency')['value']}}"></i>{{sprintf("%.2f",($row->ori_offer_price - $row->tax_amount / $row->qty) *$conversion_rate,2)}}

                                      @else

                                        <i class="{{session()->get('currency')['value']}}"></i>
                                        {{sprintf("%.2f",($row->ori_price - $row->tax_amount / $row->qty) *$conversion_rate,2)}}
                                ...
                                <p class="pro-des pro-des-one"> <b>{{__('staticwords.Tax')}} :</b>
                                  ...
                                  @if($row->simple_product->store->state->id != $address->getstate->id)
                                    ...
                                    {{ sprintf("%.2f",($row->tax_amount / $row->qty) * $conversion_rate) }} <b>[IGST]</b>
                                  ...
                                  @if($row->simple_product->store->state['id'] == $address->getstate->id)
                                    ...
                                    @endphp
                                    {{ sprintf("%.2f",$diviedtax * $conversion_rate) }} <b>[SGST]</b> &nbsp; | &nbsp;
                                    <i class="fa {{ Session::get('currency')['value'] }}"></i>
                                    {{ sprintf("%.2f",$diviedtax * $conversion_rate) }} <b>[CGST]</b>
                                  @endif

                                  @else
                                    {{ ($row->tax_amount / $row->qty) * $conversion_rate }} <b> [{{ $row->simple_product->tax }}% ({{ $row->simple_product->tax_name }})]</b>
                                  @endif
                        ...
                        <div class="text-right col-12 offset-md-3 col-md-9 offset-xl-0 col-xl-3">
                          <p><b>
                                ...
                                + {{sprintf("%.2f",(($row->semi_total - $row->tax_amount) *$conversion_rate))}}
                                @else

                                + {{ sprintf("%.2f",(($row->price_total- $row->tax_amount)*$conversion_rate))}} <i class="{{session()->get('currency')['value'] }}"></i>
                          ...
                          <p>
                            <b>+&nbsp;
                              <i class="fa {{ Session::get('currency')['value'] }}"></i>
                              {{ sprintf("%.2f",$row->tax_amount * $conversion_rate) }}
                            </b>
                            <small class="text-muted"><b>( {{__('staticwords.TotalTax')}} )</b></small>
                            {{-- End if Product Tax 0  --}}
                          </p>
                          ...
                          <div class="finaltotalbox">
                            ...
                            <b>
                              ...
                              <span id="totalprice{{ $row->id }}">
                                ...
                                @if($row->semi_total != '' && $row->semi_total != 0)
                                  {{ sprintf("%.2f",($row->semi_total+$row->gift_pkg_charge+$row->shipping)*$conversion_rate,2) }}
                                @else
                                {{ sprintf("%.2f",($row->price_total+$row->gift_pkg_charge+$row->shipping)*$conversion_rate) }}
                                @endif
                              </span>
                    ...
                    <div class="cart-sub-total totals-value" id="cart-total">
                      <div class="row">
                        ...
                        <div class="col-md-8 col-5">
                          <span class="" id="show-total">
                            <i class="{{session()->get('currency')['value']}}"></i>{{  $total_gift_pkg_charge }}
                          </span>
                        </div>
                      </div>
                    </div>
