	...
	<div class="container-fluid">
		...
		<div class="row justify-content-md-center">
			<div class="printheader padding-15">
				<br>
				<a href="{{ route('show.order',$getInvoice->order->order_id) }}" title="Go back" class="p_btn2 pull-left btn btn-md btn-default"><i
					...
					<tbody>
						...
						<tr>
							<td colspan="2">
								...
								$ci = App\Allcity::where('id',$store->city_id)->first() ?
								App\Allcity::where('id',$store->city_id)->first()->name : '';
						...
						$country = App\Allcity::where('id',$address->city_id)->first() ?
						...
						<tr>
							...
							<td>
								<p><b>{{ $getInvoice->order->billing_address['firstname'] }},
										{{ $getInvoice->order->billing_address['mobile'] }}</b></p>
								<p class="font-weight">{{ strip_tags($getInvoice->order->billing_address['address']) }},
								</p>
								@php


								$bcountry =
								App\Allcountry::where('id',$getInvoice->order->billing_address['country_id'])->first()->nicename;
								$bstate =
								App\Allstate::where('id',$getInvoice->order->billing_address['state'])->first() ?
					...
					<tbody>
						<tr>
							<td>
									...
									<small class="tax"><b>{{ __('Price:') }}</b> <i
											class="{{ $getInvoice->order->paid_in }}"></i>
							...
							<td>
								...
								<p><i class="{{ $getInvoice->order->paid_in }}"></i>
									{{ sprintf("%.2f",$getInvoice->igst) }}</p>
								@endif
								@if($getInvoice->sgst != NULL)
								<p><i class="{{ $getInvoice->order->paid_in }}"></i>
									{{ sprintf("%.2f",$getInvoice->sgst) }} (SGST)</p>
								@endif
								@if($getInvoice->cgst != NULL)
								<p><i class="{{ $getInvoice->order->paid_in }}"></i>
									{{ sprintf("%.2f",$getInvoice->cgst) }} (CGST)</p>
								@endif
								<p><b>Total:</b> <i
										class="{{ $getInvoice->order->paid_in }}"></i>{{ round($getInvoice->tax_amount,2) }}
								</p>
						...
						<tr>
							...
							<td>
								@if( $getInvoice->order->discount == 0)
								<i class="{{ $getInvoice->order->paid_in }}"></i>
								{{ round( $getInvoice->qty*$getInvoice->price+$getInvoice->tax_amount+$getInvoice->handlingcharge+$getInvoice->shipping+$getInvoice->gift_charge,2) }}
								@else
								<i class="{{ $getInvoice->order->paid_in }}"></i>
								{{ round( $getInvoice->qty*$getInvoice->price+$getInvoice->tax_amount-$getInvoice->discount+$getInvoice->handlingcharge+$getInvoice->shipping+$getInvoice->gift_charge,2) }}
								@endif
							</td>
						</tr>
					</tbody>
